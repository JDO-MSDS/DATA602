...
def exercise12(n):
    ...
    '''
    # ------ Place code below here \/ \/ \/ ------
    checkerboard_matrix = (np.indices((2 * n, 2 * n)).sum(axis=0) + 1) % 2
    # ------ Place code above here /\ /\ /\ ------ 
...
def exercise13(n):
    ...
    plt.title("Cumulative Sum of Random Integers from 1/1/2010")
    plt.show()
    # ------ Place code above here /\ /\ /\ ------ 
    return s
...
def exercise14(words):
    ...
    # ------ Place code below here \/ \/ \/ ------
    se = pd.Series(words)
    df = se.map(len).to_frame(name="length")
    # ------ Place code above here /\ /\ /\ ------ 
    return df
...
def exercise15():
    ...
    '''
    # ------ Place code below here \/ \/ \/ ------
    df_original, _, _, _, _ = exercise08()
    df = df_original.iloc[::5][["street", "zip"]]
    # ------ Place code above here /\ /\ /\ ------ 
    return df
